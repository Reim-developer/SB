...
class UnBlackListPrefix(commands.Cog):
	...
	def __missing_args_embed(
		...
		embed = Embed(
			description = (
				...
				f"* Usage:\n" \
				f"* `{prefix()}` unblacklist `[USER_ID]` `(REASON)`\n" \
				f"* [] -> required argument\n" \
				f"* () -> optional argument\n" \
				f"* Example:\n" \
				f"* `{prefix()} unblacklist 1234 successful appeal`"
			),
			color = 0xc1cfde,
	...
	def __success_embed(self, user: User, reason: Optional[str]) -> Embed:
		embed = Embed(
			description = (
				f"* Successfully remove blacklist: {user.name}\n" \
				f"* User ID: `{user.id}`\n" \
	...
	def __not_in_blacklist(self, user: User) -> Embed:
		embed = Embed(
			description = (
				f"* User: {user.name} | `{user.id}` is not blacklisted"
			),
			color = 0xfac0b4,
			timestamp = datetime.now()
		)
	...
	async def unblacklist(
		...
		if blacklist_user:
			is_already_blacklist = await self.__blacklist_manager.is_blacklisted(user_id = user.id)

			if is_already_blacklist == BlacklistResult.NOT_BLACKLISTED:
				await ctx.channel.send(embed = self.__not_in_blacklist(user = blacklist_user))
				return

			await self.__blacklist_manager.remove_blacklist_user(user_id = blacklist_user.id)
			...
			if self.LOG_CHANNEL and isinstance(self.LOG_CHANNEL, TextChannel):
				...
				await self.LOG_CHANNEL.send(content = (
					f"**__Remove Blacklist Notification:__**\n" \
...
async def setup(bot: commands.Bot) -> None:
	await bot.add_cog(UnBlackListPrefix(bot = bot))